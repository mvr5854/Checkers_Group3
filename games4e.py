...
# Adaptions to work with the notebook games4e.ipynb because
# Functions defined in the class in games4e.py are similar 
# but NOT exactly same as how they defined in games4e.ipynb.

# def get_caller_module():
#     for frame in inspect.stack():
#         module_name = frame.frame.f_globals.get("__name__")
#         # Skip internal importlib modules and look for the first actual caller
#         if module_name and not module_name.startswith("importlib"):
#             # print('caller', module_name)
#             return module_name
#     return None  # Fallback if nothing is found


# caller = get_caller_module()
# is_terminal = lambda game, state : game.is_terminal(state) if caller == "games4e" else game.terminal_test(state)
# to_move = lambda game, state : state.to_move if caller == "games4e" else game.to_move(state)
...
def monte_carlo_tree_search(state, game, N=1000):
    # Get the name of the module that is calling the function.
    def select(n):
    ...
    def simulate(game, state):
        ...
        while not game.terminal_test(state):
            if not list(game.actions(state)):
...
if __name__ == "__main__":
    g = ConnectFour()
    g.play_game(mcts_player, alpha_beta_cutoff_player)